...
plt.title(r'LES MM, $\overline{\Delta}_{0} = \frac{2\pi}{64}$, $2 \nu \bar{S}_{ij}\bar{S}_{ij}$ (LES)') 
...
plt.title(r'LES MM, $\overline{\Delta}_{0} = \frac{2\pi}{64}$,  $\epsilon_{exact}$ vs  $2\nu_t\bar{S}_{ij}\bar{S}_{ij}$')
...
plt.title(r'LES MM, $\overline{\Delta}_{0} = \frac{2\pi}{64}$, $\nu_t$')
...
plt.figure(2)
plt.savefig('turbulence_production_2pi_64.png')
