...
time_32,  MeanKEDR_32  , nu32 , pk32  = data_32[:, 0], data_32[:, 2] , data_32[:, 4] , data_32[:, 5]
...
time_256, MeanKEDR_256 , nu256, pk256 = data_256[:, 0], data_256[:, 2], data_256[:, 4], data_256[:, 5]

dnstime_256, dnsMeanKEDR_256, dnsfMeanKEDR_256 = dnsdata_256[:, 0], dnsdata_256[:, 2], dnsdata_256[:, 4]
#ntime_256, nMeanKEDR_256, nu256, npk256, s256 = ndata_256[:, 0], ndata_256[:, 2], ndata_256[:, 4], ndata_256[:, 5], ndata_256[:, 6] 
...
plt.title(r'LES smg, $\bar{\Delta} = \Delta x  $ $2 \nu \bar{S}_{ij}\bar{S}_{ij}$ (LES)') 

plt.plot(time_32, MeanKEDR_32,color='C3',  label=r'$32^3$')
plt.plot(time_64, MeanKEDR_64,color='C0', label=r'$64^3$')
plt.plot(time_128, MeanKEDR_128,color='C1', label=r'$128^3$')
plt.plot(time_256, MeanKEDR_256,color='C2', label=r'$256^3$')
...
plt.title(r'LES smg, $\bar{\Delta}$  = $\Delta x$ ,  $\epsilon_{exact}$ vs  $2\nu_t\bar{S}_{ij}\bar{S}_{ij}$')
plt.plot(time_256, pk256, color='C6', label=r'$2\nu_t\bar{S}_{ij}\bar{S}_{ij}(les)$')
...
plt.title(r' LES smg, $\bar{\Delta}$  = $\Delta x$ , $\nu_t$')
plt.plot(time_256, nu256, color='C6', label=r'$\nu_t$  $256^3 $')
plt.xlabel('$time(s)$', fontsize=14)
plt.ylabel(r'$\nu_t $', fontsize=14)
plt.legend(fontsize=12, frameon=True, borderpad=1.5, handletextpad=1.5, 
           markerscale=2, edgecolor='black', framealpha=1)
...
plt.figure(1)
plt.savefig('mean_kedr_les_smg_revolved_fields_delta_dx.png')

plt.figure(2)
plt.savefig('turbulence_production_smg_tf15.png')

plt.figure(3)
plt.savefig('turbulence_viscosity_smg_tf15.png')
