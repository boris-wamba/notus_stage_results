...
plt.title(r'LES MM, $\bar{\Delta} = \Delta x  $ $2 \nu \bar{S}_{ij}\bar{S}_{ij}$ (LES)') 
...
plt.title(r'LES MM, $\bar{\Delta}$  = $\Delta x$ ,  $\epsilon_{exact}$ vs  $2\nu_t\bar{S}_{ij}\bar{S}_{ij}$')
...
plt.title(r' LES MM, $\bar{\Delta}$  = $\Delta x$ , $\nu_t$')
...
plt.figure(1)
plt.savefig('mean_kedr_les_mm_revolved_fields_delta_dx.png')

plt.figure(2)
plt.savefig('turbulence_production_mm_tf15.png')

plt.figure(3)
plt.savefig('turbulence_viscosity_mm_tf15.png')
